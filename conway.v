...
Definition element_last (e : Element) : Sym :=
  match last_sym (element_to_word e) with
  | Some s => s
  | None => S1
  end.

Definition element_first (e : Element) : Sym :=
  match first_sym (element_to_word e) with
  | Some s => s
  | None => S1
  end.
...
Lemma adjacent_boundaries_ok_app_aux : forall es1 es2,
...
Proof.
  ...
  - intros es2 _ _ Hne1 _ _.
    exfalso.
    apply Hne1.
    reflexivity.
  - intros es2 Hadj1 Hadj2 _ Hne2 Hcross.
...
Lemma last_sym_last_symbol : forall w, last_sym w = last_symbol w.
Proof.
  intros w.
  unfold last_symbol.
  induction w as [|x xs IH].
...
Lemma first_sym_first_symbol : forall w, first_sym w = first_symbol w.
Proof.
  intros w.
...
Proof.
  ...
  - unfold boundaries_differ.
    rewrite <- last_sym_last_symbol.
    unfold element_last in Hbdiff.
    destruct w as [|y ys]; [contradiction|].
    simpl.
    destruct (last_sym (element_to_word e)) as [s|] eqn:Hls.
    + exact Hbdiff.
    + destruct e; vm_compute in Hls; discriminate.
Qed.
...
Lemma flat_map_decay_adjacent_boundaries_ok : forall es,
...
Proof.
  ...
  - intros w1 w2 Hsplit Hne1 Hne2 Hni.
    ...
    + unfold find_split_point in Hatom.
      ...
      * discriminate.
      * assert (Hlen1 : length w1 >= 1).
        { destruct w1; [contradiction | simpl; lia]. }
        assert (Hlen2 : length w2 >= 1).
        { destruct w2; [contradiction | simpl; lia]. }
        assert (Hlensum : length w1 + length w2 = length (x :: y :: ys)).
        { rewrite Hsplit. rewrite app_length. reflexivity. }
        assert (Hbound : length w1 <= length (x :: y :: ys) - 1).
        { simpl in Hlensum. simpl. lia. }
        assert (Hfirstn : firstn (length w1) (x :: y :: ys) = w1).
        { rewrite Hsplit.
          clear.
          induction w1 as [|a w1' IH].
          - reflexivity.
          - simpl. f_equal. apply IH. }
        assert (Hskipn : skipn (length w1) (x :: y :: ys) = w2).
        { rewrite Hsplit.
          clear.
          induction w1 as [|a w1' IH].
          - reflexivity.
          - simpl. apply IH. }
        assert (Hvalid : valid_split_at (x :: y :: ys) (length w1) splitting_depth_bound = true).
        { unfold valid_split_at.
          rewrite Hfirstn. rewrite Hskipn.
          destruct w1; [contradiction|].
          destruct w2; [contradiction|].
          exact Hni. }
        assert (Hfound : exists k, find_split_point_aux (x :: y :: ys) (length (x :: y :: ys) - 1) splitting_depth_bound = Some k).
        { clear Hfind.
          assert (Hsearch : forall m, m >= length w1 -> m <= length (x :: y :: ys) - 1 ->
            exists k, find_split_point_aux (x :: y :: ys) m splitting_depth_bound = Some k).
          { induction m as [|m' IHm].
            - intros Hge _. lia.
            - intros Hge Hle.
              simpl.
              destruct (valid_split_at (x :: y :: ys) (Datatypes.S m') splitting_depth_bound) eqn:Hcheck.
              + exists (Datatypes.S m'). reflexivity.
              + destruct (Nat.eq_dec (Datatypes.S m') (length w1)) as [Heq' | Hneq].
                * rewrite Heq' in Hcheck. rewrite Hvalid in Hcheck. discriminate.
                * apply IHm; lia. }
          apply Hsearch; lia. }
        destruct Hfound as [k Hcontra].
...
Lemma iterate_decay_boundaries_10 : forall n e,
  n <= 10 ->
...
Lemma iterate_audio_iterate_decay_connection : forall n e,
  n <= 10 ->
  iterate_audio n (element_to_word e) = elements_to_word (iterate_decay n [e]).
Proof.
  intros n e Hle.
...
Lemma iterate_audio_element_no_Sd : forall n e,
  n <= 10 ->
  word_contains_Sd (iterate_audio n (element_to_word e)) = false.
Proof.
  intros n e Hle.
  rewrite iterate_audio_iterate_decay_connection.
  - apply iterate_decay_no_Sd.
  - exact Hle.
Qed.

Definition is_reachable_pair (e1 e2 : Element) : bool :=
  is_decay_pair (e1, e2) ||
  match e1, e2 with
  | Li, He => true | H, H => true | He, He => true | Li, H => true
  | H, K => true | K, He => true | Th, H => true | Lu, Th => true
  | Ga, K => true | Nd, Ne => true | Dy, Al => true | Dy, Th => true
  | K, Fe => true | V, Al => true | Cu, Fe => true | Sm, K => true
  | K, Ra => true | Ra, H => true | K, Cu => true | Dy, Eu => true
  | Ga, Ne => true | Rb, Pa => true | U, H => true | K, Mo => true
  | Pm, Y => true | Dy, Nd => true | Nd, In => true | K, Pm => true
  | Ba, H => true | Nd, K => true | K, Ta => true | Nd, Tl => true
  | Dy, Po => true | H, Dy => true | H, Tb => true | He, Tb => true
  | Li, Tb => true | He, Dy => true | Li, Dy => true | He, Ni => true
  | Li, Ni => true | Nb, H => true | Ta, H => true | He, Cu => true
  | Li, Cu => true | He, Zn => true | Li, Zn => true | He, Ho => true
  | Li, Ho => true | N, Cu => true | N, Tb => true | N, Ho => true
  | O, Tb => true | O, Dy => true | O, Zn => true | F, Dy => true
  | F, Ho => true | F, Ni => true | Ne, Cu => true | Ne, Tb => true
  | Ne, Ho => true | Na, Tb => true | Na, Dy => true | Na, Zn => true
  | Mg, Dy => true | Mg, Ho => true | Mg, Ni => true | Al, Tb => true
  | Al, Ho => true | Al, Ge => true | Al, Cu => true | Si, Tb => true
  | Si, Dy => true | Si, Zn => true | P, Dy => true | P, Ni => true
  | S, Ho => true | S, Cu => true | B, Tb => true | B, Dy => true
  | B, Zn => true | C, Dy => true | C, Ho => true | C, Ni => true
  | Fe, Ho => true | Fe, Ba => true | Fe, C => true | Fe, B => true
  | Fe, P => true | Fe, V => true | Fe, Se => true | Fe, Pr => true
  | Fe, Rh => true | Fe, Ir => true | Fe, Rn => true | Co, Cr => true
  | Co, Nd => true | Co, P => true | Co, Fr => true | Co, La => true
  | Co, Br => true | Co, C => true | Co, N => true | Co, S => true
  | Co, Pd => true | Co, Pt => true | Ni, Mn => true | Ni, S => true
  | Ni, N => true | Zn, Co => true | Zn, Ar => true | Zn, F => true
  | Mn, Dy => true | Mn, Ho => true | Mn, As => true | Mn, B => true
  | Mn, Be => true | Mn, Ce => true | Mn, Cs => true | Mn, Os => true
  | Mn, Ti => true | Ar, Fr => true | Ar, Hf => true | Ar, Mn => true
  | Ar, Nb => true | Ar, Nd => true | Ar, Ni => true | Cl, Cr => true
  | Cl, Er => true | Cl, Lu => true | Cl, Pr => true | Cl, Rn => true
  | Cl, Zn => true | Si, Ba => true | Si, Be => true | Si, Er => true
  | Si, Ge => true | Si, La => true | Si, Re => true | Si, Sc => true
  | Si, Xe => true | P, La => true | P, Ti => true | P, Tm => true
  | S, Ce => true | S, V => true | S, Yb => true | Ce, Ag => true
  | Ce, Au => true | Ce, Cl => true | Ce, Kr => true | Ce, O => true
  | Ce, S => true | Pr, Ar => true | Pr, Cd => true | Pr, Cl => true
  | Pr, F => true | Pr, Hg => true | Pr, Rb => true | Gd, Ce => true
  | Gd, Pm => true | Gd, Ra => true | Tb, Ac => true | Tb, Bi => true
  | Tb, Mg => true | Tb, Pr => true | Tb, Sm => true | Er, Ca => true
  | Tm, Ra => true | Yb, Ac => true | Kr, Th => true | Sc, Pm => true
  | Ti, Mg => true | As, Fr => true | Se, Ra => true | Br, Ac => true
  | Mg, Cs => true | Mg, Xe => true | Al, I => true | Al, Ba => true
  | Al, Cs => true | Na, I => true | Na, Xe => true | Na, Rn => true
  | Ne, I => true | H, Ar => true | H, Cl => true | H, P => true
  | H, S => true | H, Ca => true | H, Ho => true | Be, Cu => true
  | Be, Ho => true | Be, Tb => true | Nd, Ar => true | Nd, Sr => true
  | Pm, K => true | Pm, Ca => true | Pm, Na => true | Pm, Sn => true
  | Pm, Pb => true | Sm, K => true | _, _ => false
  end.

Lemma reachable_pair_cross_ok : forall e1 e2,
  is_reachable_pair e1 e2 = true ->
  negb (sym_eqb (element_last (decay_last_element e1))
                (element_first (decay_first_element e2))) = true.
Proof.
  intros e1 e2 Hr.
  destruct e1, e2; vm_compute in Hr |- *; try reflexivity; try discriminate.
Qed.

Lemma decay_pair_is_reachable : forall e1 e2,
  is_decay_pair (e1, e2) = true -> is_reachable_pair e1 e2 = true.
Proof.
  intros e1 e2 Hdp.
  unfold is_reachable_pair.
  rewrite Hdp.
  reflexivity.
Qed.

Lemma reachable_cross_is_reachable : forall e1 e2,
  is_reachable_pair e1 e2 = true ->
  is_reachable_pair (decay_last_element e1) (decay_first_element e2) = true.
Proof.
  intros e1 e2 Hr.
  destruct e1, e2; vm_compute in Hr |- *; try reflexivity; discriminate.
Qed.
